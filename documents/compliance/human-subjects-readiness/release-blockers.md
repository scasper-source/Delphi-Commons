...
|---|---|---|---|---|---|
| HSB-P0-001 | HSR-018 | Session/security deployment hardening is not yet launch-grade. | Core backend membership enforcement now exists, but human-subjects launch still requires security review of session storage, CSRF posture, rate limits, deployment secrets, and production configuration. | Session/security design approved; CSRF/rate-limit/security-header/secret-handling tests pass; deployment docs show session-required mode; residual risks accepted by Security & Privacy Lead. | Security & Privacy Lead |
| HSB-P0-002 | HSR-004, HSR-019 | Retention, deletion request, backup, and restore workflows are incomplete. | Study teams must know how records are retained, restored, and handled after withdrawal/deletion requests. | Retention policy enforcement, deletion/withdrawal review, backup/restore runbook, and restore rehearsal evidence exist. | Data Custodian |
| HSB-P0-003 | HSR-017 | WCAG 2.2 AA accessibility evidence is incomplete. | Participants must be able to use consent and study tasks accessibly. | Automated and manual accessibility audit completed; P0/P1 accessibility defects resolved or documented with approved mitigation. | Study Owner / Accessibility Reviewer |
| HSB-P0-004 | HSR-018 | Security hardening evidence is incomplete. | Sensitive participant data requires hardened sessions, rate limits, headers, input handling, dependency review, and abuse tests. | OWASP ASVS checklist completed for chosen assurance level; critical/high issues resolved or formally accepted. | Security & Privacy Lead |

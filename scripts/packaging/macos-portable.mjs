...
import { buildRuntimeMetadata, generateInventory, buildChecksums, scanForbiddenMaterial, scanOverclaimText, buildManifest, generateEvidenceTemplate, sha256File } from './core/index.mjs';
...
const packageInventory = (r) => generateInventory(r).filter((item) => item !== 'package-manifest.json');
...
function stageProdDeps() { fs.rmSync(productionDepsRoot, { recursive: true, force: true }); fs.mkdirSync(productionDepsRoot, { recursive: true }); copyPath(path.join(repoRoot, 'server/package.json'), path.join(productionDepsRoot, 'package.json')); copyPath(path.join(repoRoot, 'server/package-lock.json'), path.join(productionDepsRoot, 'package-lock.json')); run(npmCommand, ['--prefix', productionDepsRoot, 'ci', '--omit=dev']); return path.join(productionDepsRoot, 'node_modules'); }
...
async function buildPackage() { const config = buildMacosAdapterConfig(); const raw = readJson(runtimeMetaPath); const runtimeMetadata = buildMacosRuntimeMetadata(raw); fs.mkdirSync(packageRoot, { recursive: true }); run(npmCommand, ['--prefix', 'app', 'run', 'build']); run(npmCommand, ['--prefix', 'server', 'run', 'build']); const prod = stageProdDeps(); copyPath(path.join(repoRoot, 'app/dist'), path.join(packageRoot, 'app/dist')); copyPath(path.join(repoRoot, 'server/dist'), path.join(packageRoot, 'server/dist')); copyPath(path.join(repoRoot, 'server/package.json'), path.join(packageRoot, 'server/package.json')); copyPath(path.join(repoRoot, 'server/package-lock.json'), path.join(packageRoot, 'server/package-lock.json')); copyPath(prod, path.join(packageRoot, 'server/node_modules')); copyPath(path.join(repoRoot, 'scripts/macos/portable-bundled-runtime.sh'), path.join(packageRoot, 'scripts/macos/portable-bundled-runtime.sh')); copyPath(path.join(repoRoot, 'scripts/macos/portable-operator-candidate.sh'), path.join(packageRoot, 'scripts/macos/portable-operator-candidate.sh')); copyPath(path.join(repoRoot, 'scripts/windows/static-file-server.mjs'), path.join(packageRoot, 'tools/static-file-server.mjs')); copyPath(path.join(repoRoot, 'LICENSE'), path.join(packageRoot, 'LICENSE')); copyPath(path.join(repoRoot, 'NOTICE'), path.join(packageRoot, 'NOTICE')); await stageRuntime(raw); fs.writeFileSync(path.join(packageRoot, 'README.txt'), buildBundledReadme(), 'utf8'); generateEvidenceTemplate({ platform: 'macOS', outputPath: path.join(packageRoot, 'evidence-template.md') }); assertNoAmbient(packageRoot); const inventory = packageInventory(packageRoot); const checksums = buildChecksums(packageRoot, inventory); const manifest = buildManifest({ config, inventory, checksums, runtimeMetadata, packageName, packageVersion: '0.0.0-internal', commitHash: execSync('git rev-parse HEAD', { cwd: repoRoot }).toString().trim(), limitations: ['Internal synthetic testing only.', 'Intel Mac x64 runtime metadata/package deferred in this change.', 'Local Node/npm required at build time.'], nonClaims: ['No signing/notarization/Gatekeeper readiness claim.', 'No macOS support-readiness claim.', 'No pilot/production/human-testing readiness claim.'] }); if (scanForbiddenMaterial(inventory).length) throw new Error('Forbidden material present.'); if (scanOverclaimText(JSON.stringify(manifest)).length) throw new Error('Overclaim scan failed.'); fs.writeFileSync(path.join(packageRoot, 'package-manifest.json'), JSON.stringify(manifest, null, 2)); fs.mkdirSync(outRoot, { recursive: true }); fs.writeFileSync(latestPackageRootFile, packageRoot); }
function verifyPackage() { const runtimeRoot = process.env.EDELPHI_RUNTIME_ROOT || path.resolve(packageRoot, '..', 'runtime-state'); const result = verifyPackageEvidence({ packageRoot, runtimeRoot, overclaimFiles: ['README.txt', 'evidence-template.md', 'package-manifest.json'] }); if (!result.ok) throw new Error(`Package verification failed:\n- ${result.failures.join('\n- ')}`); }

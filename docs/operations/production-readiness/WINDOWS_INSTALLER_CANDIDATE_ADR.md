...
- Installer-candidate launchers resolve package root from `scripts/windows/installer-candidate/*.vbs` by walking four parent levels and route lifecycle commands through `scripts/windows/installer-candidate-bundled-runtime.ps1` (thin wrapper; no lifecycle duplication).

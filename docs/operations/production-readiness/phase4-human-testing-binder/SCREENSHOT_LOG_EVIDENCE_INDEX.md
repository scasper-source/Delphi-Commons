...
| F4-EVID-020 | Screenshot/log | Main Menu, Study Workspace Launcher, saved-study reopen, and post-relaunch continuity | CORRECTED_PACKAGE_RELEASED_PENDING_RETEST | HUMAN_REQUIRED | Corrected `r10` package exists with Main Menu, launcher, fresh New Study reset, and multi-study persistence coverage; clean Windows screenshots/logs are still required. |
| F4-EVID-021 | Screenshot/log | Study PI and Ethics PI signoff controls | CORRECTED_PACKAGE_RELEASED_PENDING_RETEST | HUMAN_REQUIRED | Corrected `r10` package exists with backend regression and clearer UI; clean Windows screenshots/logs are still required. |

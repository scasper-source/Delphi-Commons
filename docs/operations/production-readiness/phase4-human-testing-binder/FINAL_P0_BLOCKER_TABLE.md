...
| F4-P0-008 | Auth/session owner + Study Owner/operator | OPEN / CORRECTED_PACKAGE_RELEASED_PENDING_RETEST | `../WINDOWS_GITHUB_INSTALLER_RELEASE_ASSET_EVIDENCE_2026-05-31.md`, `SCREENSHOT_LOG_EVIDENCE_INDEX.md`, `server/tests/multipleStudyPersistence.test.mjs`, `app/tests/policyGates.test.mjs` | 2026-05-29 clean Windows `r8` report shows create/save and dashboard list passing, but saved-study reopen was not proven and post-relaunch saved/new-study continuity failed or was unclear. Corrected `r10` package now includes package-mode multi-study persistence coverage, the Study Workspace Launcher, Main Menu first screen, and fresh New Study reset behavior. | Rerun the Main Menu, launcher, steps 8, 9, 10, and post-relaunch smoke against `r10` before any human-use readiness mark. |
| F4-P0-009 | Governance owner + Study Owner/operator | OPEN / CORRECTED_PACKAGE_RELEASED_PENDING_RETEST | `../WINDOWS_GITHUB_INSTALLER_RELEASE_ASSET_EVIDENCE_2026-05-31.md`, `SCREENSHOT_LOG_EVIDENCE_INDEX.md`, `server/tests/signoffGovernanceFlow.test.mjs`, `app/tests/policyGates.test.mjs` | 2026-05-29 clean Windows `r8` report indicates Study Owner/PI and Ethics & Methods/PI signoff controls appear broken, unreachable, or not wired clearly enough for the operator workflow. Corrected `r10` package verifies backend role/sequence boundaries and clarifies UI labels, assignment guidance, and disabled-action reasons. | Retest governance signoff from the `r10` packaged app before any human-use readiness mark. |

...
3. Start backend and UI using documented operator commands.
   - Confirm the operator localhost URL (`127.0.0.1`) is displayed.
   - Keep LAN/tunnel mode disabled unless explicitly testing internal synthetic phone-flow access.
   - For LAN mode, set both `EDELPHI_ENABLE_LAN_PARTICIPANT_URL=1` and `EDELPHI_ACK_LAN_SYNTHETIC_ONLY=1`, then capture acknowledgement in notes.
...
It also does **not** claim real SMS/provider readiness, public internet safety, or participant real-device evidence without separately attached human/device artifacts.

...
| Laptop installer candidates | Windows installer adapter and macOS installer adapter are merged. GitHub release assets exist for the Windows installer ZIP/EXE. The prior macOS split `.pkg`/portable ZIP assets are superseded for installer/operator testing; the corrected macOS path must be one installer ZIP bundle containing the PKG and instructions. | Installer packaging is downloadable from GitHub for internal use only after the corrected asset exists, but installed lifecycle, signing, reputation/Gatekeeper, and clean-profile evidence remain open. |
...
- **RECORDED (2026-05-18):** GitHub Actions macOS release workflow succeeded and created a prerelease with Apple Silicon internal package assets: a portable bundled-runtime ZIP and unsigned internal installer `.pkg`. Evidence: [MACOS_GITHUB_RELEASE_ASSET_EVIDENCE_2026-05-18.md](./MACOS_GITHUB_RELEASE_ASSET_EVIDENCE_2026-05-18.md). This split release surface is superseded for macOS installer/operator testing. The corrected path is one installer ZIP bundle containing the PKG, verification metadata, checksums, and instructions. Installed-path lifecycle, signing, notarization, Gatekeeper, Intel Mac/x64, and operator walkthrough evidence remain open.
- **RECORDED (2026-05-18):** GitHub Actions Windows installer release workflow succeeded and created a prerelease with Windows x64 internal installer assets: a recommended installer ZIP bundle, raw unsigned installer EXE, checksums, package manifest, and package verification metadata. The first successful installer asset `r2` was superseded and deleted after a user-reported Windows code 193 post-install launch failure; `r3` was superseded and deleted after a user-reported browser-reopen/desktop-shortcut launch issue; `r4` was superseded and deleted after the installed package still exposed user-facing Stop/Status shortcuts and a background-runtime mental model; `r5` was superseded and deleted after stale Stop wording remained in the generated ZIP README; `r6` was superseded and deleted after review found browser-fallback order and attached-browser-unavailable cleanup issues. Current `r7` opens Delphi Commons through one user-facing Desktop/Start Menu launcher, uses an attached Edge or Chrome app window, and stops the local runtime when that Delphi Commons browser app window closes. Evidence: [WINDOWS_GITHUB_INSTALLER_RELEASE_ASSET_EVIDENCE_2026-05-18.md](./WINDOWS_GITHUB_INSTALLER_RELEASE_ASSET_EVIDENCE_2026-05-18.md). This makes Windows installer assets available from GitHub, but clean-profile/second-machine installed lifecycle, signing, SmartScreen, Defender, enterprise deployment, and operator walkthrough evidence remain open.
...
- Capture install/start/browser-open/browser-window-close-to-stop/uninstall evidence from the downloaded Windows installer ZIP bundle on a clean Windows profile or second Windows machine.

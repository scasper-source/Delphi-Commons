...
| Laptop installer candidates | Windows installer adapter and macOS installer adapter are merged. GitHub release assets now exist for the Windows installer ZIP/EXE and macOS Apple Silicon `.pkg`/portable ZIP. | Installer packaging is now downloadable from GitHub for internal use, but installed lifecycle, signing, reputation/Gatekeeper, and clean-profile evidence remain open. |

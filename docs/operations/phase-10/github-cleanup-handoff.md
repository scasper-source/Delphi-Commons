...
Status: PARTIAL. This handoff lists follow-up work only. It does not modify GitHub community files.

## Revisit Before GitHub Migration

- `README.md`: confirm readiness language, install commands, screenshots if added, and no production claim.
- `LICENSE`: confirm Apache-2.0 text.
- `NOTICE`: confirm attribution.
- `SECURITY.md`: replace `[security contact placeholder]` with a safe reporting path or GitHub Security Advisory instructions.
- `CONTRIBUTING.md`: confirm no real participant data or secrets are accepted.
- `GOVERNANCE.md`: confirm Charter supremacy and AI thin-spec relationship.
- `CODE_OF_CONDUCT.md`: NOT FOUND; decide whether to add.
- Issue templates: NOT FOUND; add security/privacy-safe templates later.
- Pull request template: NOT FOUND; add checks for tests, docs, privacy, AI governance, and no real data.
- Release checklist: existing [Open Source Release Checklist](../../OPEN_SOURCE_RELEASE_CHECKLIST.md) should be updated after Phase 10 rehearsal evidence exists.
- `.env.example`: NOT FOUND; add placeholders only if the project wants a checked-in example.

...
| `EDELPHI_TWILIO_CONNECT_URL` | `server/src/core/config.ts`, `server/src/routes/sms.ts` | Twilio Messaging Services Console fallback | Optional operator-facing Twilio Connect or setup URL surfaced by the SMS setup prompt. Do not include secrets in this URL. |
